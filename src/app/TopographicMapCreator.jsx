...
// Generate biome map based on seed with smooth blending
const generateBiomeMap = (width, height, seedValue, biomes) => {
  if (biomes.length === 1) {
  ...
  const getBiomeNoise = (x, y) => {
    ...
    for (let o = 0; o < octaves; o++) {
      ...
      const n00 = seededRandom(seedValue * 7 + x0 * 127.1 + y0 * 311.7 + o * 1000);
      const n10 = seededRandom(seedValue * 7 + x1 * 127.1 + y0 * 311.7 + o * 1000);
      const n01 = seededRandom(seedValue * 7 + x0 * 127.1 + y1 * 311.7 + o * 1000);
      const n11 = seededRandom(seedValue * 7 + x1 * 127.1 + y1 * 311.7 + o * 1000);
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      // Map noise to biome index with smooth transitions
      const biomeIndex = Math.floor(noise * biomes.length) % biomes.length;
      biomeMap[y][x] = biomes[Math.abs(biomeIndex)];
    }
  }
...
// Blend colors between two hex colors
...
// Generate biome blend map for smooth color transitions
const generateBiomeBlendMap = (width, height, seedValue, biomes) => {
  if (biomes.length === 1) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      const noise = getBiomeNoise(x, y);
      const scaledNoise = noise * biomes.length;
      const biomeIndex1 = Math.floor(scaledNoise) % biomes.length;
      const biomeIndex2 = (biomeIndex1 + 1) % biomes.length;
      const blendRatio = scaledNoise - Math.floor(scaledNoise);
      
      // Smooth the blend ratio for softer transitions
      const smoothBlend = blendRatio * blendRatio * (3 - 2 * blendRatio);
      
      blendMap[y][x] = {
        biome1: biomes[Math.abs(biomeIndex1)],
        biome2: biomes[Math.abs(biomeIndex2)],
        blend: smoothBlend
      };
    }
  }
...
// Get blended elevation color
const getBlendedElevationColor = (elevation, blendInfo) => {
  if (!blendInfo) {
    return getElevationColor(elevation, BIOME_TYPES.TEMPERATE);
  }
  
  const color1 = getElevationColor(elevation, blendInfo.biome1);
  const color2 = getElevationColor(elevation, blendInfo.biome2);
  
  return blendColors(color1, color2, blendInfo.blend);
};
...
export default function TopographicMapCreator() {
  ...
  const handleSizeSelection = useCallback((size) => {
    ...
    setTimeout(() => {
      ...
      // Initialize biome maps
      const initSeed = Math.random() * 100000;
      biomeMapRef.current = generateBiomeMap(selectedSize.width, selectedSize.height, initSeed, selectedBiomes);
      biomeBlendMapRef.current = generateBiomeBlendMap(selectedSize.width, selectedSize.height, initSeed, selectedBiomes);
        ...
        for (let dx = -radius; dx <= radius; dx++) {
          ...
          if (dist <= radius) {
            ...
            if (py >= 0 && py < data.length && px >= 0 && px < data[0].length) {
              ...
              if (drawModeRef.current === DrawMode.RAISE) {
                const nextLevel = elevationLevels.find(e => e > currentElev + 1);
                if (nextLevel) {
                  data[py][px] = currentElev + (nextLevel - currentElev) * falloff * BRUSH_CONSTANTS.ELEVATION_BLEND;
                }
              } else if (drawModeRef.current === DrawMode.LOWER) {
                const prevLevel = [...elevationLevels].reverse().find(e => e < currentElev - 1);
                if (prevLevel) {
                  data[py][px] = currentElev + (prevLevel - currentElev) * falloff * BRUSH_CONSTANTS.ELEVATION_BLEND;
                }
  ...
  const handleGenerateFromSeed = useCallback(() => {
    const seedNum = parseFloat(inputSeed);
    if (isNaN(seedNum)) {
      alert('Please enter a valid number for the seed');
      return;
    }
    generateRandomMap(seedNum);
  }, [inputSeed, generateRandomMap]);
